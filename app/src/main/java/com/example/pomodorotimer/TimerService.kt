...
class TimerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                currentTotalTime = totalTimeMs
                currentIsBreak = isBreakMode
    ...
    private fun sendTickBroadcast(timeLeft: Long) {
        sendBroadcast(Intent(ACTION_TICK).apply {
            ...
            putExtra(EXTRA_TOTAL_TIME, totalTimeMs)
        })
    }
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            CHANNEL_ID, "Помодоро таймер", NotificationManager.IMPORTANCE_LOW
        ).apply {
            description = "Уведомление таймера"
            setShowBadge(false)
            setSound(null, null)
        }
    ...
    private fun updateNotification(timeLeft: Long, total: Long, playing: Boolean) {
        getSystemService(NotificationManager::class.java)
            .notify(NOTIFICATION_ID, buildNotification(timeLeft, total, playing))
    }
